    ...
    <div className="space-y-4">
      <section className="rounded-xl border border-[#E5E5E5] bg-white p-5">
        <div className="border-b border-[#E5E5E5] pb-4">
          <p className="text-[16px] font-bold text-[#0A0A0A]">새 상담 기록 추가</p>
          <p className="mt-1 text-[13px] leading-[1.6] text-[#737373]">후속 통화가 끝나면 여기만 바로 채우면 돼요. 저장하면 아래 목록 상단에 즉시 반영돼요.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <label className="block">
              <span className="mb-2 block text-[13px] font-medium text-[#0A0A0A]">상담일시</span>
            ...
            <label className="block">
              <span className="mb-2 block text-[13px] font-medium text-[#0A0A0A]">상담 채널</span>
          ...
          <label className="block">
            <span className="mb-2 block text-[13px] font-medium text-[#0A0A0A]">상담 요약</span>
          ...
          <label className="block">
            <span className="mb-2 block text-[13px] font-medium text-[#0A0A0A]">상세 메모</span>
      ...
      {consultations.length > 0 ? consultations.map((consultation) => (
        <article key={consultation.id} className="rounded-xl border border-[#E5E5E5] bg-white p-5">
          <div className="flex flex-col gap-2 border-b border-[#E5E5E5] pb-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-[15px] font-semibold text-[#0A0A0A]">{consultation.consultedAt}</p>
              <p className="mt-0.5 text-[13px] text-[#A3A3A3]">{consultation.channel}</p>
            </div>
            <p className="text-[15px] font-medium text-[#0A0A0A]">{consultation.summary}</p>
          </div>
          <p className="mt-3 text-[15px] leading-[1.6] text-[#737373]">{consultation.details || "-"}</p>
        </article>
      )) : (
