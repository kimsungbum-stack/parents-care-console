...
export function AiAnalysisCard({ leadId, latestConsultationDetails, planTier }: AiAnalysisCardProps) {
  ...
  if (!canUseAi) {
    return (
      <div className="rounded-xl border border-dashed border-[#E7E0D5] bg-white p-4 sm:p-5">
        <div className="flex items-center gap-2">
          <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-[#F5F5F4] text-[#A8A29E]">
            <Lock size={14} />
          </div>
          <p className="text-[13px] font-medium text-[#78716C]">AI 상담 분석</p>
        </div>
        <p className="mt-2 text-[15px] leading-[1.6] text-[#78716C]">
  ...
  if (!latestConsultationDetails) {
    return (
      <div className="rounded-xl border border-dashed border-[#E7E0D5] bg-white p-4 sm:p-5">
        ...
        <p className="mt-2 text-[13px] text-[#A8A29E]">상담 기록이 있으면 AI가 자동으로 분석할 수 있어요.</p>
      </div>
    );
  }
  ...
  if (!result) {
    return (
      <div className="rounded-xl border border-[#E7E0D5] bg-white p-4 sm:p-5">
        ...
        <p className="mt-2 text-[13px] leading-[1.6] text-[#78716C]">최근 상담 내용을 AI가 분석하여 보호자 정보, 상황 요약, 핵심 이슈를 추출해요.</p>
        {error && (
  ...
  return (
    <div className="rounded-xl border border-[#E7E0D5] bg-white p-4 sm:p-5">
      <div className="flex items-center gap-2">
        ...
        <p className="text-[13px] font-medium text-[#78716C]">AI 상담 분석 결과</p>
      </div>

      <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-start sm:gap-6">
        <div className="min-w-0 flex-1">
          <p className="text-[15px] font-semibold text-[#292524]">{result.guardianName}</p>
          {result.careTarget && (
      ...
      <div className="mt-3 rounded-lg border border-[#E7E0D5] bg-[#FEFCF8] p-3 sm:p-4">
        ...
        <p className="mt-1 text-[13px] leading-[1.6] text-[#78716C]">{result.situationSummary}</p>
      </div>

      {result.keyIssues.length > 0 && (
        <div className="mt-3">
          <p className="text-[13px] font-medium text-[#A8A29E]">핵심 이슈</p>
          <ul className="mt-2 flex flex-wrap gap-2">
            {result.keyIssues.map((issue) => (
              <li
                key={issue}
                className="rounded-full border border-[#E7E0D5] bg-[#FEFCF8] px-2.5 py-1 text-[13px] text-[#292524]"
              >
                {issue}
              </li>
            ))}
