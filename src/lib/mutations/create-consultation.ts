...
export async function createLeadConsultation(
  ...
  if (!isSupabaseConfigured()) {
    return {
      status: "error",
      message:
        "데이터 연결이 아직 완료되지 않아 상담 기록을 저장할 수 없어요.",
    };
  }
  ...
  try {
    ...
    if (leadError) {
      return {
        status: "error",
        message:
          "상담 기록을 저장하기 전에 케이스 정보를 확인하지 못했어요.",
      };
    }
    ...
    if (!leadRow) {
      return {
        status: "not_found",
        message: "해당 케이스를 찾을 수 없어 상담 기록을 저장하지 못했어요.",
      };
    }
    ...
    if (consultationError || !consultationRow) {
      return {
        status: "error",
        message:
          "상담 기록을 저장하지 못했어요. 연결 상태를 확인해 주세요.",
      };
    }
    ...
    return {
      status: "error",
      message:
        "상담 기록 저장 중 문제가 발생했어요. 잠시 후 다시 시도해 주세요.",
    };
  }
}
